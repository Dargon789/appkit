import { c as we, n as z } from './D1u8rsYX.js'
import './D5z3hTdP.js'
import { aw as de, X as ge, N as he, Q as j, O as me, as as x } from './cxC4FtgZ.js'

import { n as z, c as we } from './D1u8rsYX.js'
import './D5z3hTdP.js'
var K = {},
  tt,
  Tt
        ...
        ;(o.fromArray = function (f) {
          ...
          (o.fromString = function (f, g) {
            const l = A(f, a.isKanjiModeEnabled()),
              P = S(l),
              y = w(P, g),
              N = u.find_path(y.map, 'start', 'end'),
              R = []
            for (let p = 1; p < N.length - 1; p++) R.push(y.table[N[p]].node)
...
function qe() {
  ...
  function d(y, N, R) {
    const p = y.size,
      M = m.getEncodedBits(N, R)
    let E, b
    for (E = 0; E < 15; E++)
      (b = ((M >> E) & 1) === 1),
        E < 6 ? y.set(E, 8, b, !0) : E < 8 ? y.set(E + 1, 8, b, !0) : y.set(p - 15 + E, 8, b, !0),
        E < 8
          ? y.set(8, p - E - 1, b, !0)
          : E < 9
            ? y.set(8, 15 - E - 1 + 1, b, !0)
            : y.set(8, 15 - E - 1, b, !0)
    y.set(p - 8, 8, 1, !0)
  }
  function f(y, N) {
  ...
  function g(y, N, R) {
    ...
    const M = o.getSymbolTotalCodewords(y),
      E = a.getTotalCodewordsCount(y, N),
      b = (M - E) * 8
    for (p.getLengthInBits() + 4 <= b && p.put(0, 4); p.getLengthInBits() % 8 !== 0; ) p.putBit(0)
    const B = (b - p.getLengthInBits()) / 8
    for (let T = 0; T < B; T++) p.put(T % 2 ? 17 : 236, 8)
    return l(p, y, N)
  }
  function l(y, N, R) {
    const p = o.getSymbolTotalCodewords(N),
      M = a.getTotalCodewordsCount(N, R),
      E = p - M,
      b = a.getBlocksCount(N, R),
      B = p % b,
      T = b - B,
      F = Math.floor(p / b),
      H = Math.floor(E / b),
    ...
    let Mt = 0,
      D,
      _
    for (D = 0; D < W; D++) for (_ = 0; _ < b; _++) D < Y[_].length && (X[Mt++] = Y[_][D])
    for (D = 0; D < bt; D++) for (_ = 0; _ < b; _++) X[Mt++] = St[_][D]
    return X
  }
  function P(y, N, R, p) {
    let M
...
`
    ...
    return (
      ...
      s.applyMask(p, T),
      d(T, R, p),
      { modules: T, version: N, errorCorrectionLevel: R, maskPattern: p, segments: M }
    )
  }
  return (
    (et.create = function (N, R) {
